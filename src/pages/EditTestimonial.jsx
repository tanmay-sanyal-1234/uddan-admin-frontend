...
import { useUpdateTestimonial ,useGetTestimonialDetails} from "@/hooks/blogHook";
...
function EditTestimonials() {
    ...
    const { mutateAsync: updateTestimonial, isPending:isPendingBlogPublish } = useUpdateTestimonial(id);
    const { data: useGetTestimonialDetailsData, isFetching } = useGetTestimonialDetails(id);
    ...
    useEffect(() => {
        if(useGetTestimonialDetailsData?.success && !isFetching){
            const details = useGetTestimonialDetailsData?.data;
            setFormData({
                name: details?.name || "",
    ...
    },[useGetTestimonialDetailsData,isFetching])
